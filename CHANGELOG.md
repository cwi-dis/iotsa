...
- `tests/Led`/`tests/KitchenSink` now reference `vanilla_*` role aliases instead of hardcoding a specific board for each chip family, so repointing a family's default board only touches the alias, not every test entry; each also dropped `lolin32` as a redundant second classic-ESP32 board (no build-relevant difference from `esp32thing`, unlike e.g. `esp32c3devkit`/`esp32c3supermini` which genuinely differ) -- `lolin32` stays covered via `sandbox/BLEClient` (#222)
