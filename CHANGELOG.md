...
- HPS now supports chunked request bodies up to 8192 bytes (previously hard-capped at the 512-byte BLE ATT attribute limit), reassembled firmware-side and sent transparently by the Python client; bodies over that still fail fast with a clear error instead of silently losing the write and crashing later (#139)
