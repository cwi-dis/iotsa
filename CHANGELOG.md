...
- `-DESP32S3` was never injected for esp32s3 boards, unlike the equivalent `-DESP32C3` for esp32c3 -- the exact inconsistency #222 named as an example of accumulated board-section drift; nothing currently reads it (code uses the ESP-IDF-provided `CONFIG_IDF_TARGET_ESP32S3` instead), but it's now there for anyone following the existing `#ifdef ESP32C3` convention (#222)
...
- `tests/Led`'s variant matrix rebuilt from 16 accumulated (and partly duplicate) rows down to 11 deliberate ones: one `all` (every optional transport this board supports) per chip family, `onlycoap`/`onlyhttps`/`nonetworking` on `vanilla_esp8266`+`vanilla_esp32`, and `onlyble` on `vanilla_esp32s3` (#222)
...
- `tests/Led`/`tests/KitchenSink`, and now every `examples/*/iotsa-build.json` too, reference `vanilla_*` role aliases instead of hardcoding a specific board for each chip family, so repointing a family's default board only touches the alias, not every entry (no effect on generated output -- aliases resolve before any file is generated); each also dropped `lolin32` as a redundant second classic-ESP32 board (no build-relevant difference from `esp32thing`, unlike e.g. `esp32c3devkit`/`esp32c3supermini` which genuinely differ) -- `lolin32` stays covered via `sandbox/BLEClient` (#222)
- Generated env/job names reordered `name-kind-label-board` (was `board-kind-name`, e.g. `Led-test-all-esp32c3devkit` not `esp32c3devkit-test-Led-all`): name+kind+label together identify *what* is being built, board trails as a "compile for this target" qualifier; `tests`/`sandbox` load ahead of `examples` too, so a foundational break surfaces on an early CI job instead of waiting behind the much longer examples/ tail (#222)
- Board definitions split out of `platformio.ini` into `iotsa-board-traits.ini` (hand-authored layer 3) and generated `iotsa-board-defs.ini` (layers 1-2, new `--format=board-defs-ini`); `BOARD_INFO` in `gen_build_matrix.py` now also carries each board's own `build_flags`/`partitions`/`mcu`/`flash_size`, the single source both the CI matrix and standalone per-example `platformio.ini` now pull from (see Fixed above) (#222)
