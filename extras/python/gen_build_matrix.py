...
def emit_board_defs_ini(out):
    ...
    out.write(f"; Generated {datetime.date.today().isoformat()} by "
              "extras/python/gen_build_matrix.py from iotsa-board.json --\n")
    ...
    out.write(";\n")
    out.write("; Vendored into an app repo? The date above is this copy's vintage -- once in\n")
    out.write("; a while, diff -c it against the current iotsa iotsa-board-defs.ini for\n")
    out.write("; relevant changes. Never edit a board section here downstream: put\n")
    out.write("; app-specific board choices in the app's own platformio.ini. A section here\n")
    out.write("; that isn't in iotsa's copy is the signal it still needs porting back.\n")
