...
def emit_standalone_ini(entries, target_dir, out):
    ...
    if not own:
        print(f"error: no iotsa-build.json entries found for {rel}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        elif args.format == "standalone-ini":
            if not args.dir:
                ap.error("--format=standalone-ini requires --dir")
            emit_standalone_ini(entries, os.path.join(REPO_ROOT, args.dir), out)
    finally:
        if args.output:
            out.close()
